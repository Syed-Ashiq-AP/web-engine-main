    ...
    React.useEffect(() => {
        document.addEventListener("loadAttributes", (e: Event) => {
            ...
            for (let i = 0; i < styleAttr.length; i++) {
                const propName = styleAttr[i];
                const value = styleAttr.getPropertyValue(propName);

                if (typeof value === "string" && value.length > 0) {
                    styles[kebabToCamel(propName)] = value;
                }
            }
            ...
            const tagAttributes = Object.keys(
                nativeAttributeControllers
            ).filter((attr) =>
                nativeAttributeControllers[
            ...
            tagAttributes.forEach((attr) =>
                nativeAttributeControllers[
                    ...
                    (atr: any) =>
                        (newAttributes[atr] = element.getAttribute(atr))
                )
            );
                    ...
                    <AccordionContent className="flex flex-col p-4 border rounded-lg gap-4">
                        <Combobox
                            ...
                            items={EventListeners.map((listener) => ({
                                ...
                                (x) =>
                                    !Object.keys(listenerControlls).includes(
                                        x.value
                                    )
                            )}
                            onSelect={(val: any) => {
                                setListenerControlls((prev) => ({
                                    ...prev,
                                    [val]: "",
                                }));
                            }}
                        ...
                        />
                        {Object.entries(listenerControlls).map(
                            ([lis, name], i) => (
                                <ListenController
                                    key={i}
            ...
            <Accordion type="single" collapsible>
                {styleControlls.map((control, index) => (
                    <AccordionItem value={control} key={index}>
                        ...
                        <AccordionContent>
                            {React.createElement(
                                styleControllers[control as StyleControllerKeys]
                                    .Root,
                                {
                                    setStyles,
                                    styles,
