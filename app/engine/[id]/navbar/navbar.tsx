...
type MenuHeading = "Blocks" | "Layers" | "Classes" | "Animations" | "Scripts";

const actionMenu: { heading: string; icon: React.ReactNode }[] = [
  {
    heading: "Save",
...
export const Navbar = () => {
  ...
  const { activeMenu, setActiveMenu } = pageContext;
  ...
  return (
    <Dock show={show}>
      {actionMenu.map((item, i) => (
        <DockIcon key={i} heading={item.heading} onClick={() => saveData()}>
          {item.icon}
        </DockIcon>
      ))}
      {menu.map((item, i) => (
